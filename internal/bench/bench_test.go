...
func TestScenarioRegistryIsStableAndUnique(t *testing.T) {
	want := []string{
		...
		"approval-unavailable", "approval-once", "session-timeout",
	}
...
func TestUnavailableDockerIsSkipAndFailClosedStillRuns(t *testing.T) {
	...
	if report.Version != SchemaVersion || report.Summary.Passed != 1 || report.Summary.Failed != 0 || report.Summary.Skipped != 21 {
