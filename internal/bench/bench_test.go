...
func TestScenarioRegistryIsStableAndUnique(t *testing.T) {
	want := []string{
		...
		"approval-unavailable", "approval-once", "approval-containment-precedence", "concurrent-approval-once", "cross-session-security-isolation", "session-timeout",
	}
...
func TestUnavailableDockerIsSkipAndFailClosedStillRuns(t *testing.T) {
	...
	if report.Version != SchemaVersion || report.Summary.Passed != 1 || report.Summary.Failed != 0 || report.Summary.Skipped != 24 {
