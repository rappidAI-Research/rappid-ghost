...
- **CLI:** validates command shape, selects the configured runtime, turns categorized setup failures into concise guidance, and presents a deterministic security summary from stored session events. It does not construct decoy values, SQL, Docker arguments, or a parallel reporting truth source.
...
- **Session manager:** owns status transitions, policy evaluation, synthetic-home preparation, runtime invocation, and routing runtime evidence through the security-signal pipeline. It journals cleanup responsibility before a recoverable runtime can create Docker objects and atomically commits terminal state with `SESSION_END`. A per-project process lock prevents a live session from being mistaken for interrupted recovery work. Incidents are reconstructed later and are not separately persisted.
...
- **Runtime:** exposes a minimal `Run` operation plus optional preflight and fail-closed recovery capabilities. Docker remains the only production implementation. Its preflight validates the existing command, policy, workspace, synthetic-home/session state, Docker availability, non-root identity, and exact digest-pinned image, pulling that same reference once when absent, then returns a single-use prepared execution. A shared confinement profile supplies the non-root identity, Docker's isolated PID namespace, explicit private IPC/cgroup namespaces, capability drop, `no-new-privileges`, core-dump prohibition, read-only root, and per-role PID limit to the agent and both sidecars. The result can carry access evidence for explicit Shadow resources and reports whether post-run cleanup could be verified.
...
7. Ask the Docker runtime to preflight its mandatory static boundary. A failure records the session as failed without `PROCESS_START` and without invoking the prepared execution. On success, record `PROCESS_START`, making command-scope exposure to the mounted selected workspace sources derivable without claiming a file read, and consume the prepared execution exactly once.
...
15. Re-read that session's persisted events and render either a quiet no-action result or a compact security summary. The summary counts only supported evidence types and session state; it neither copies event metadata nor constructs causal narratives.

The static preflight deliberately reuses Docker runtime validation instead of duplicating security rules in the CLI. Image preparation, sentinel readiness, network creation, gateway health, and container creation still occur in the prepared execution because they allocate live resources. Each remains fail closed before the agent container can run; a setup failure never triggers host execution or a weaker retry. `PROCESS_START` represents Ghost's durable handoff to that prepared runtime, not proof that Docker ultimately created a guest process.
...
Failures after session creation still transition the session to `failed` and leave an event trail. Final-state persistence uses a short context detached from command cancellation, so an interrupted agent does not normally leave its session marked `running`. SQLite commits terminal state and its sole `SESSION_END` in one transaction; an identical retry is idempotent and a different terminal rewrite is rejected. The runtime verifies that an allowlist gateway is still running before accepting an otherwise completed run. If Ghost itself terminates before finalization, the next run removes only positively identified resources for that recorded session, retains its containment flag, marks it `failed`, and adds a recovery `SESSION_END`. If Docker cleanup could not be verified after terminal persistence, the durable cleanup-pending marker causes the next run to retry the same strict reconciliation without changing the recorded outcome. Docker, sentinel, network, gateway, or recovery failure never invokes the command on the host.
...
Schema migration 4 adds an internal `cleanup_pending` recovery journal and enforces one `SESSION_END` per session. Non-terminal `created`/`running` rows remain recovery candidates; terminal rows are included only when cleanup was not verified. Recovery preserves the recorded terminal outcome and containment state while clearing that marker only after exact owned-resource cleanup succeeds.
...
The prepared Docker execution snapshots validated `runtime.limits` and starts one bounded execution context before allocating live resources. All container roles share hard confinement plus memory/swap and CPU caps. The agent container is created without auto-removal and its retained HostConfig is validated before start. A distinct non-root keeper holds the cgroup open; the agent executes inside that same container under its original identity. Trusted kernel OOM counters are read before launch and before teardown, and Docker exec inspection confirms actual process start/exit. Cancellation targets the complete container tree, followed by removal by immutable ID.
