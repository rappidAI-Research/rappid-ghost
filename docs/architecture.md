...
- **Runtime:** exposes a minimal `Run` operation plus optional preflight and fail-closed recovery capabilities. Docker remains the only production implementation. Its preflight validates the existing command, policy, workspace, synthetic-home/session state, Docker availability, and non-root identity exactly once and returns a single-use prepared execution. A shared confinement profile supplies the non-root identity, Docker's isolated PID namespace, explicit private IPC/cgroup namespaces, capability drop, `no-new-privileges`, core-dump prohibition, read-only root, and per-role PID limit to the agent and both sidecars. The result can carry access evidence for explicit Shadow resources.
