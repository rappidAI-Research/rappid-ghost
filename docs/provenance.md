...
`SECURITY_SIGNAL` and `SIGNALED` provide a secret-minimized representation for the v0.3 structured-signal vocabulary. They are present only when a corresponding stored event exists. Startup observations link their normalized `workspace:<path>` resource to the signal. `PROCESS_START` records the durable launch request before Docker setup completes. It alone cannot create a process node or exposure. A subsequent matching process exit or supported runtime access/request must confirm execution. `STARTED` is derived from both event references, and `EXPOSED_TO` additionally requires the earlier source-observation reference; neither invents a process read. A `SENSITIVE_RESOURCE_REQUESTED` signal creates a derived command-scope `REQUESTED` edge only when it references a matching, earlier `DECOY_ACCESS` event for the same protected path.
