...
It skips `.git`, `.ghost`, dependency/build outputs, test fixtures, symlinks, non-regular files, binary or invalid UTF-8 files, and unrecognized source files. Discovery and source reads are confined to an `os.Root`. Nonblocking opens and identity/type checks reject replaced sources without waiting on a FIFO. Directory listings are bounded before allocation; a directory larger than the remaining entry budget is omitted with explicit truncation, rather than scanning an arbitrary filesystem-order subset. The default bounds are 256 KiB per file, 4 MiB total analyzed text, 256 candidate files, 20,000 discovered entries, and 4,096 analyzed lines per file. Recognized agent-instruction files are considered before lower-priority documentation or scripts. For a line-heavy file, only bounded leading and trailing regions are analyzed. Limit activation is recorded as `RESOURCE_LIMIT_TRIGGERED`; it is not silently presented as a complete scan.
...
The policy evaluation context can carry `PROMPT_INJECTION_SUSPECTED`, but Ghost does not terminate a run or generate ASK solely because text matched. For a destination already configured as approvable, the finding can appear as factual context in the user explanation. It cannot make an `ALLOW`, `DENY`, `SHADOW`, or `ASK` decision more permissive, make a forbidden operation approvable, or establish that the content caused the request. Docker isolation, network policy, environment isolation, Shadow resources, and containment remain authoritative even when the guard misses a prompt injection. If the inspector itself cannot initialize or complete safely, Ghost records a failed session and does not start the runtime.
