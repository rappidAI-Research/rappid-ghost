...
The production Docker runtime validates mandatory prerequisites before the session manager records `PROCESS_START`: the deterministic network policy, canonical workspace boundary, synthetic-home/session paths, selected SHADOW paths, Docker CLI/daemon availability, numeric non-root identity, and exact digest-pinned image. An absent image is obtained through one bounded Docker pull of that same reference and verified before preparation completes. That validation returns a single-use prepared execution, preventing normal orchestration from inspecting Docker twice or drifting from the runtime's own rules. Failure marks the session failed and does not invoke the prepared execution. Live container, sentinel, gateway, and network construction follows inside that execution and also fails closed before the agent can run; there is no host, floating-image, or weakened-runtime retry.
...
Session directories retain the synthetic home and structured observation log locally for auditability. They use private directory permissions and are not mounted into later sessions. Before a recoverable runtime can create Docker objects, Ghost durably marks cleanup pending. Normal cleanup removes the agent and sidecars plus both temporary networks, then verifies that no exactly owned objects remain; daemon unavailability is not treated as proof of cleanup. Terminal state and the sole `SESSION_END` event commit atomically, while the cleanup marker remains set when verification failed. A per-project advisory lock prevents concurrent `ghost run` processes from confusing live and interrupted state. On the next run, Ghost queries non-terminal rows and terminal cleanup-pending rows, then removes only Docker objects whose session label, component label, and exact expected name agree. It preserves containment and any terminal outcome, and clears the marker only after reconciliation succeeds. Ambiguous ownership, unavailable Docker, or cleanup failure aborts the new run. SQLite lock waits are bounded so an external database writer causes a prompt visible failure instead of an indefinite-looking stall.

## Dependency boundary and limitations

Docker supplies the isolation boundary; Ghost does not protect against a compromised daemon, image, kernel, or container escape. Ghost may ask Docker to pull the source-pinned Alpine image during preflight before execution. A deny guest remains network-disabled; an allowlist guest receives only the internal gateway path described above.
...
- A hard crash may leave labeled agent, gateway, sentinel, or network objects until the next successful project recovery; Ghost neither scans nor deletes objects it cannot tie to a non-terminal or cleanup-pending session in that project's database.
