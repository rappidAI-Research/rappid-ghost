...
The integrated Prompt-Injection Guard emits one `UNTRUSTED_CONTENT_OBSERVED` event for each selected source successfully analyzed during bounded startup inspection, and emits `PROMPT_INJECTION_SUSPECTED` when deterministic rules match. It may emit `RESOURCE_LIMIT_TRIGGERED` when a scan bound prevents complete analysis. A genuine Shadow open/access also produces a `SENSITIVE_RESOURCE_REQUESTED` signal that references the source `DECOY_ACCESS` event; this is a derived resource-class statement, not evidence that Ghost inspected a real secret. The Docker runtime also emits `RESOURCE_LIMIT_TRIGGERED` for confirmed OOM, sampled process saturation, and session deadlines. Resource-limit events are operational `OBSERVATION` category signals, not proof of malicious behavior; they do not create hostile incidents or containment by themselves. Metadata uses a bounded kind, numeric limit/observation, and `classification: operational`. The vocabulary does not imply byte-level taint tracking, universal resource-abuse prevention, or model reasoning access. See [runtime resources](runtime-resources.md).
